...
def index():
    '''
    Выводит список всех Собатий всех Пользователей
    '''
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Если пользователь поставит галку remember_me, то сессия
        # будет сохранятся и после закрытия браузера
...
def event(event_id):
    '''
    Изменения События Пользователя
    '''
    ...
    # Определяем, принадлежит ли Событие текущему Пользователем
    event_owner = False
    if current_user.get_id() and event.user_id == int(current_user.get_id()):
        event_owner = True

    # Если События не текущего Пользователя, то гасим ввод в форму
    if not event_owner:
        form.disable_form()

    if form.validate_on_submit():
        # Для непосредственной команды изменения (POST) проверяем принадлежность
        # События текущему Пользователю
        if not event_owner:
    ...
    # eid будет определять, показывать ли кнопку удаления События
...
def delevent(event_id):
    '''
    Удаление События
    Похоже на def event, но не валидирует форму, т.к. для удаления это не нужно
    '''
    event = Event.query.filter_by(id=event_id).first()
    if not event:
        flash('Event does not exist', 'error')
